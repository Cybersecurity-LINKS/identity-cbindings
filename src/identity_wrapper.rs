...
use identity_iota::{
    core::{json, FromJson, Object, OneOrMany, ToJson},
    credential::{
        Credential, CredentialBuilder, FailFast, Jwt, JwtCredentialValidationOptions,
        JwtCredentialValidator, JwtCredentialValidatorUtils, Subject,
    },
    did::DID,
    ...
    verification::MethodRelationship,
};
...
use iota_sdk::{
    client::{api::GetAddressesOptions, stronghold::StrongholdAdapter},
    types::block::output::AliasOutput,
    Url,
};
...
impl Wallet {
    pub async fn setup(stronghold_path: &str, password: &str) -> anyhow::Result<Self> {
        let client_node =
            env::var("IOTA_NODE_ENDPOINT").expect("IOTA_NODE_ENDPOINT not found in ENV");
        let client_2 = Client::builder()
            .with_primary_node(&client_node, None)?
            .finish()
            .await;
        ...
        let faucet_endpoint =
            env::var("FAUCET_ENDPOINT").expect("FAUCET_ENDPOINT not found in ENV");
        let address = Self::request_funds_for_address(
            None,
            &client,
        ...
        Ok(Self {
            ...
            address,
            network,
            storage, /* , vc: None, did_document: None, fragment: None, peer_did_document: None */
        })
    }

    async fn setup_secret_manager(
        stronghold_path: &str,
        password: &str,
    ) -> anyhow::Result<StrongholdAdapter> {
...
impl Did {
    pub async fn did_create(wallet: &Wallet) -> anyhow::Result<Self> {
        ...
        let alias_output: AliasOutput = wallet
            .client
            .new_did_output(wallet.address, document, None)
            .await?;

        // Publish the Alias Output and get the published DID document.
        let did_document = wallet
            .client
            .publish_did_output(wallet.stronghold_storage.as_secret_manager(), alias_output)
            .await?;
        let doc_json = did_document.to_json()?;

        Self::write_on_file(
            DID_OID,
            &privkey.to_json()?.as_str(),
            &fragment.as_str(),
            &doc_json,
            "did_document.json",
        )?;
    ...
    pub async fn get_did(&self) -> anyhow::Result<String> {
        Ok(self.fragment.to_owned()
            + " "
            + &self.privkey.to_json()?
            + " "
            + &self.did_document.to_json()?
            + " ")
    }
    ...
    pub async fn did_sign(&self, wallet: &Wallet, message: &[u8]) -> anyhow::Result<Vec<u8>> {
        ...
        let sig: Vec<u8> = self
            ...
            .create_sig(
                &wallet.storage,
                &self.privkey.clone().expect("Hello"),
                &self.fragment,
                message,
    ...
    pub async fn did_verify(&self, sig: &[u8], signing_input: &[u8]) -> anyhow::Result<()> {
        ...
        let result = self.did_document.as_ref().verify_sig(
            kid,
            signature,
            signing_input,
...
impl Vc {
    pub async fn vc_create(wallet: &mut Wallet, did: &Did, name: &str) -> anyhow::Result<Self> {
        ...
        let alias_output: AliasOutput = wallet
            .client
            .new_did_output(wallet.address, issuer_document, None)
            .await
            .unwrap();

        // Publish the Alias Output and get the published DID document.
        let issuer_document: IotaDocument = wallet
            .client
            .publish_did_output(wallet.stronghold_storage.as_secret_manager(), alias_output)
            .await
            .unwrap();
    ...
    pub async fn vc_verify(wallet: &Wallet, peer_vc: &str) -> anyhow::Result<Did> {
        ...
        let peer_did = match &decoded_vc.credential.credential_subject {
            OneOrMany::One(ref credential_subject) => credential_subject
                .id
                .as_ref()
        ...
        println!(
            "endpoint did resolve time = {} micro\n",
            elapsed4.as_micros()
        );

        let fragment = String::from("Hello");
        let peer_did = Did {
            did_document: peer_did_doc,
            fragment: fragment,
            privkey: None,
        };
